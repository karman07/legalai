    ...
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-slate-900 flex items-center">
          <LibraryIcon className="w-7 h-7 mr-2 text-gold-600" />
          My Library
        </h2>
        ...
        <div className="space-y-4">
          {savedChats.length === 0 ? (
            ...
            savedChats.map((chat) => (
              <div
                key={chat.id}
                className="bg-slate-50 border border-brand-200 rounded-lg p-5 hover:shadow-md transition-shadow"
              >
                    ...
                    {chat.tags && chat.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {chat.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-1 bg-white text-slate-700 text-xs rounded-full border border-brand-200"
                          >
                            {tag}
                          </span>
                        ))}
            ...
            testSessions.map((session) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <button
                    className="ml-4 px-4 py-2 bg-gold-500 hover:bg-gold-600 text-white rounded-lg transition-colors text-sm font-medium"
