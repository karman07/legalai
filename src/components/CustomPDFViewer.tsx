...
export default function CustomPDFViewer({ pdf, fileUrl, onClose }: CustomPDFViewerProps) {
  ...
  // ── Selection-only TTS (separate from full-page TTS so doc stays visible) ──
  const [selTtsPlaying, setSelTtsPlaying] = useState(false);
  const [selTtsPaused, setSelTtsPaused] = useState(false);
  const selUtteranceRef = useRef<SpeechSynthesisUtterance | null>(null);
  ...
  // Speak only the selected text — does NOT touch ttsText so the document stays fully visible
  const speakSelectedOnly = useCallback(() => {
    ...
    utterance.rate = ttsRate;
    if (ttsVoice) utterance.voice = ttsVoice;
    utterance.onstart = () => { setSelTtsPlaying(true); setSelTtsPaused(false); };
    utterance.onend   = () => { setSelTtsPlaying(false); setSelTtsPaused(false); };
    utterance.onerror = () => { setSelTtsPlaying(false); setSelTtsPaused(false); };
    selUtteranceRef.current = utterance;
    window.speechSynthesis.speak(utterance);
    setSelectionPopup(null);
  }, [selectedText, ttsRate, ttsVoice]);

  const pauseSelTts = useCallback(() => {
    window.speechSynthesis.pause();
    setSelTtsPlaying(false);
    setSelTtsPaused(true);
  }, []);

  const resumeSelTts = useCallback(() => {
    window.speechSynthesis.resume();
    setSelTtsPlaying(true);
    setSelTtsPaused(false);
  }, []);

  const stopSelTts = useCallback(() => {
    window.speechSynthesis.cancel();
    setSelTtsPlaying(false);
    setSelTtsPaused(false);
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-brand-50 flex z-50 flex-col">
      ...
      {/* ── Floating Selection-TTS Controls ─────────────────── */}
      {(selTtsPlaying || selTtsPaused) && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[250] flex items-center gap-2.5 px-4 py-2.5 bg-brand-900 border border-brand-700 rounded-full shadow-2xl">
          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${selTtsPlaying ? 'bg-emerald-400 animate-pulse' : 'bg-brand-500'}`} />
          <span className="text-xs font-medium text-white whitespace-nowrap">
            {selTtsPlaying ? 'Speaking...' : 'Paused'}
          </span>
          {selTtsPlaying ? (
            <button
              onClick={pauseSelTts}
