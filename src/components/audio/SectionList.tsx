import { ArrowLeft, Search, ChevronRight, Headphones, Grid3X3, List, Play, Loader2 } from 'lucide-react';
...
export default function SectionList({
  ...
  onLoadMoreSections, onSelectSection, onBack,
}: SectionListProps) {
  ...
  const SectionCard = ({ section, isGrid }: { section: AudioSectionSlim; isGrid: boolean }) => {
    const hasAudio = section.hasEnglishAudio || section.hasHindiAudio || section.hasEasyEnglishAudio || section.hasEasyHindiAudio;
        ...
        <div className="group relative bg-white border-2 border-brand-200 rounded-2xl overflow-hidden hover:border-amber-400 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="relative p-6">
            <div className="flex items-start gap-4 mb-4">
              <div className="w-14 h-14 bg-gold-100 group-hover:bg-gold-500 rounded-xl flex items-center justify-center font-bold text-xl text-gold-700 group-hover:text-white transition-all flex-shrink-0">
                {section._index + 1}
              </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              {section.totalSubsections > 0 && (
                <div className="flex items-center gap-1.5 bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
                  <List className="w-3.5 h-3.5 text-gold-600" />
                  <span className="text-xs font-medium text-gold-700">{section.totalSubsections} parts</span>
                </div>
              )}
              {hasAudio && (
                <div className="flex items-center gap-1.5 bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
                  <Headphones className="w-3.5 h-3.5 text-gold-600" />
                  <span className="text-xs font-medium text-gold-700">Audio</span>
                </div>
              )}
            </div>
    ...
    return (
      ...
      >
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 bg-gold-100 group-hover:bg-gold-500 rounded-xl flex items-center justify-center font-bold text-xl text-gold-700 group-hover:text-white transition-all flex-shrink-0">
            {section._index + 1}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-lg text-slate-900 mb-2 group-hover:text-gold-700 transition-colors">{section.title}</h3>
            {section.englishTextPreview && (
              <p className="text-sm text-slate-600 mb-3 leading-relaxed line-clamp-2">{section.englishTextPreview}</p>
            )}
            <div className="flex items-center gap-4 text-sm">
              {section.totalSubsections > 0 && (
                <div className="flex items-center gap-1.5 bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
                  <List className="w-3.5 h-3.5 text-gold-600" />
                  <span className="font-medium text-gold-700">{section.totalSubsections} subsections</span>
                </div>
              )}
              {hasAudio && (
                <div className="flex items-center gap-1.5 bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
                  <Headphones className="w-3.5 h-3.5 text-gold-600" />
                  <span className="font-medium text-gold-700">Audio</span>
                </div>
              )}
            </div>
          </div>
          <ChevronRight className="w-6 h-6 text-brand-400 group-hover:text-gold-600 flex-shrink-0" />
        </div>
      </button>
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {lesson?.category && (
              <span className="px-3 py-1.5 bg-gradient-to-r from-gold-100 to-gold-50 text-gold-700 rounded-lg text-sm font-medium border border-gold-200">
                {lesson.category}
              </span>
            )}
            <span className="px-3 py-1.5 bg-brand-50 text-slate-700 rounded-lg text-sm font-medium border border-brand-200">
              {lesson?.totalSections ?? sections.length} sections
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
          <div className="flex flex-col lg:flex-row gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search loaded sections..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-11 pr-4 py-3 border border-brand-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
              />
            </div>
            <div className="flex bg-slate-100 rounded-lg p-1">
              <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white text-gold-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
                <Grid3X3 className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('list')} className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white text-gold-600 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
          ...
          <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-3'}>
            {filteredSections.map((section) => (
              <SectionCard key={section._index} section={section} isGrid={viewMode === 'grid'} />
            ))}
          </div>

          {filteredSections.length === 0 && !sectionsLoading && (
            <div className="text-center py-12">
              <p className="text-slate-500">{searchQuery ? `No sections found matching "${searchQuery}"` : 'Loading sections...'}</p>
            </div>
          )}
          ...
          {sectionsPage < sectionsTotalPages && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-brand-900 text-white rounded-xl hover:bg-brand-800 disabled:opacity-50 transition-all font-medium"
              >
                {sectionsLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                {sectionsLoading ? 'Loading...' : `Load more sections (${sections.length} / ${lesson?.totalSections ?? '?'})`}
              </button>
            </div>
          )}
            ...
            <div className="flex items-center gap-3 mb-4">
              {totalSubsections > 0 && (
                <div className="flex items-center gap-1.5 bg-gradient-to-r bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
                  <List className="w-3.5 h-3.5 text-gold-600" />
                  <span className="text-xs font-medium text-gold-700">{totalSubsections} parts</span>
                </div>
              )}
              {hasAudio && (
                <div className="flex items-center gap-1.5 bg-gradient-to-r bg-gold-50 px-3 py-1.5 rounded-lg border border-gold-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="px-3 py-1.5 bg-gradient-to-r from-brand-50 to-slate-100 text-slate-700 rounded-lg text-sm font-medium border border-brand-200">
