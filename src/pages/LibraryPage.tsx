...
import { BookMarked, MessageSquare, Brain, Target, FileText, Sparkles, Clock, TrendingUp, Award, Bookmark, Trash2, ChevronRight } from 'lucide-react';
...
export default function LibraryPage() {
  ...
  const getModeColor = (mode: string) => {
    switch (mode) {
      ...
      case 'custom': return 'bg-gold-100 text-gold-700 border-gold-200';
      default: return 'bg-slate-100 text-slate-700 border-brand-200';
    }
  };
      ...
      <div>
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-slate-900 flex items-center">
            <BookMarked className="w-7 h-7 mr-2 text-gold-600" />
            My Library
          </h2>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {savedChats.length === 0 ? (
              ...
              savedChats.map((chat) => (
                <div
                  key={chat.id}
                  className="bg-white border border-brand-200 rounded-lg p-5 hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        {chat.is_bookmarked && (
                          <Bookmark className="w-4 h-4 text-gold-500 fill-gold-500" />
                        )}
                      </div>
                      <p className="text-sm text-slate-600 mb-3">
                        {chat.chatbot_type === 'legal_expert' ? 'Legal Expert' : 'Legal AI Assistant'} • {' '}
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      ...
                      >
                        <Bookmark className={`w-5 h-5 ${chat.is_bookmarked ? 'text-gold-500 fill-gold-500' : 'text-brand-400'}`} />
                      </button>
                      <button
          ...
          <div className="space-y-4">
            {testSessions.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 rounded-lg">
                <Target className="w-16 h-16 text-brand-300 mx-auto mb-4" />
                ...
                <button
                  ...
                  className="mt-4 px-6 py-2 bg-gold-500 hover:bg-gold-600 text-white rounded-lg transition-colors"
                >
                  Start a Test
                </button>
              </div>
              ...
              testSessions.map((session) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <button
                      className="ml-4 px-4 py-2 bg-gold-50 hover:bg-gold-100 text-gold-700 rounded-lg transition-colors text-sm font-medium"
